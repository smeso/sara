...
static int config_dump(struct seq_file *seq)
{
	int i, ret, rulen;
	u16 path_len;
	char *buf;
	...
	buf = kmalloc(max(sizeof(*h), sizeof(*r)), GFP_KERNEL);
	...
	ret = seq_write(seq, buf, sizeof(*h));
	if (unlikely(ret))
		goto out;

	r = (struct binary_config_rule *) buf;
	for (i = 0; i < rulen; ++i) {
		rc = &c->rules[i];
		...
		ret = seq_write(seq, buf, sizeof(*r));
		if (unlikely(ret))
			goto out;
		ret = seq_write(seq, rc->path, path_len);
		if (unlikely(ret))
			goto out;
	}
